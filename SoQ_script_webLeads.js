 ...
 */
function createSageQuote(el_id){
...
/** function createAllQuotes()
 * * prepare all outstanding leads for integration
 */
function createAllQuotes() {

  updatecallTimings();
  
  let createAllActive = event.target.getAttribute('ui-active');
  if(createAllActive == "true"){

    event.target.setAttribute('ui-active', "false");
    event.target.classList.add("btn-disabled");

    let createButtons = document.querySelectorAll(".create-sage-quote");
    const numButtons = createButtons.length;
    if(numButtons < 1){
      const pall = document.createElement("P");
      const tall = document.createTextNode(`Sorry Uncle Bossman, there ain't no mo' quotes to send to the sage ...`);
      pall.appendChild(tall);
      event.target.parentElement.appendChild(pall);
    } else {

      // disable all buttons now
      for(let i=0; i < numButtons; i++){
        if ( createButtons[i].getAttribute('ui-active') == 'true' ) {
          // if not already clicked ...
          createButtons[i].classList.add('btn-disabled');
          createButtons[i].setAttribute('ui-active', 'false');
          createAllButtonsList[i] = createButtons[i];
        }
      }

      let i=0;
      createAllInterval = setInterval(function(){
        if(createAllButtonsList[i]){
          createAllButtonsList[i].setAttribute('ui-active', 'true');
          createAllButtonsList[i].click();
        }
        watchInterval(createAllInterval, i, numButtons);
        i++;
      }, 10);
...
function useAxios(callPassed){
  ...
  let axiosConfig = {
    ...
    data: JSON.stringify(leads[thisCall['xdata_index']])
  };
...
function ApiCallInit () {
  ...
  if (currentCall.length == 0 && integrationQueue.length == 0) {
    completedCalls = [];
    document.getElementById('create-all-quotes').classList.remove('btn-disabled');
    document.getElementById('create-all-quotes').setAttribute('ui-active', 'true');
  }
...
function ApiCallEnd (callRes, xdata_index) {
  ...
  currentCall[0]['end_time'] = Date.now();
  ...
  let el = document.getElementById(oldCall['el_id']);
      ...
      if (callRes) {
          if(callRes.DocumentNumber){
              ...
              pd.appendChild(td);
              el.parentElement.appendChild(pd);
